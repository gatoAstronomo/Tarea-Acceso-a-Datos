...
public class PrestamoRepositoryImpl implements PrestamoRepository {
    ...
    private static final String SELECT_BY_ID = 
        "SELECT id, usuario_id, libro_id, fecha_prestamo, fecha_devolucion_esperada, fecha_devolucion_real, estado " +
    ...
    private static final String UPDATE_SQL = 
        "UPDATE prestamos SET usuario_id = ?, libro_id = ?, fecha_prestamo = ?, " +
        "fecha_devolucion_esperada = ?, fecha_devolucion_real = ?, estado = ? WHERE id = ?";
    
    private static final String DELETE_BY_ID = 
        "DELETE FROM prestamos WHERE id = ?";
    
    private static final String SELECT_BY_USUARIO_ID = 
        "SELECT id, usuario_id, libro_id, fecha_prestamo, fecha_devolucion_esperada, fecha_devolucion_real, estado " +
        "FROM prestamos WHERE usuario_id = ?";
    
    private static final String SELECT_BY_LIBRO_ID = 
        "SELECT id, usuario_id, libro_id, fecha_prestamo, fecha_devolucion_esperada, fecha_devolucion_real, estado " +
    ...
    private static final String SELECT_ACTIVOS_BY_USUARIO_ID = 
        "SELECT id, usuario_id, libro_id, fecha_prestamo, fecha_devolucion_esperada, fecha_devolucion_real, estado " +
        "FROM prestamos WHERE usuario_id = ? AND estado = 'activo'";
    
    private static final String SELECT_ACTIVO_BY_LIBRO_ID = 
        "SELECT id, usuario_id, libro_id, fecha_prestamo, fecha_devolucion_esperada, fecha_devolucion_real, estado " +
        "FROM prestamos WHERE libro_id = ? AND estado = 'activo'";
    
    private static final String SELECT_CON_DETALLES = 
        "SELECT p.id, p.usuario_id, p.libro_id, p.fecha_prestamo, p.fecha_devolucion_esperada, " +
    ...
    private static final String DEVOLVER_SQL = 
        "UPDATE prestamos SET estado = 'devuelto', fecha_devolucion_real = CURRENT_DATE WHERE id = ?";

    // Agregar constante para existsById
    private static final String EXISTS_BY_ID = 
        "SELECT 1 FROM prestamos WHERE id = ?";

    @Override
    public Prestamo save(Connection connection, Prestamo prestamo) throws SQLException {
        logger.debug("Guardando préstamo para usuario_id: {} y libro_id: {}", prestamo.getUsuarioId(), prestamo.getLibroId());
        ...
        try (PreparedStatement stmt = connection.prepareStatement(INSERT_SQL, Statement.RETURN_GENERATED_KEYS)) {
            ...
            stmt.setDate(5, prestamo.getFechaDevolucionReal() != null ? 
                Date.valueOf(prestamo.getFechaDevolucionReal()) : null);
